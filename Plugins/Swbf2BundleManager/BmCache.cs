...
namespace BundleManager
{
    public static class BmCache
    {
        ...
        public static Dictionary<int, List<int>> BundleParents = new Dictionary<int, List<int>>();
        public static Dictionary<EbxAssetEntry, AssetData> UnmodifiedAssetData = new Dictionary<EbxAssetEntry, AssetData>();
        ...
        {
            if (!File.Exists($"{App.FileSystem.CacheName}_bundlemanager.cache"))
                return false;
            //stopWatch.Start();
            using (NativeReader reader = new NativeReader(new FileStream($"{App.FileSystem.CacheName}_bundlemanager.cache", FileMode.Open, FileAccess.Read)))
            {
